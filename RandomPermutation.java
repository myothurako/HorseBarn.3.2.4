...
public class RandomPermutation
{
  ...
  public static ArrayList<Horse> next(ArrayList<Horse> horses) {
    ...
    ArrayList<Horse> copy = new ArrayList<>();
    for (Horse h : horses) {
      copy.add(h);
    }

    // Result list
    ArrayList<Horse> randomized = new ArrayList<>();

    // Fisher–Yates style random selection
    for (int n = len; n > 0; n--) {
      int pos = (int) (Math.random() * n);
      randomized.add(copy.get(pos));
      // Replace selected element with last in copy
      copy.set(pos, copy.get(n - 1));
      copy.remove(n - 1); // remove last (already moved)
    }
